...
```{r identified-myrror, eval=FALSE}
...
After this series of warnings, the comparison proceeds as before. Again, we want to re-iterate that in the case of non-identified datasets, the comparison is done by row number (`rn`). This means that *row alignment is based on position within the dataset*, not on the content of the row, which might result in `myrror()` reporting more discrepancies than if the rows were aligned based on row content. This is also why we always suggest the user to provide keys, if possible. 
