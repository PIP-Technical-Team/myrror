...
At the end of the general overview, a ℹ️ **Note** suggests to the user to run `extract_diff_rows()` to see the differences in the number of rows. More on this function in the article [Extract Different Rows](articles/extract-diff-rows.html).
...
As suggested by the ℹ️ Note, the user can run `extract_diff_values()`/`extract_diff_table` to see the differences in list or table format. More on these functions in the articles [Extract Different Values](./extract-diff-values.html).
